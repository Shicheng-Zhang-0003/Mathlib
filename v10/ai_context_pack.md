# PROJECT BUNDLE: v10
Root Directory: /home/magi-01/Desktop/work/programming/mathlib/v10
Generated: Mon Jun 29 10:15:47 PM MDT 2026
...
static inline double ml_minimax_cos(double x) {
    // cos(x) = sin(x + pi/2). We use Payne-Hanek to shift safely.
    x = ml_reduce_payne_hanek(x + 1.5707963267948966);
    double x2 = x * x;
    double result = minimax_sin_coeffs[5];
    for (int i = 4; i >= 0; i--) result = result * x2 + minimax_sin_coeffs[i];
    return x * result;
}
...
// GCC vector extension for polynomial
...
// True AVX2 Batch RSqrt
// Uses integer bit-hacks for the initial guess, followed by TWO iterations
// of Newton-Raphson to guarantee 1e-6 precision across wide ranges.
static inline void ml_simd_batch_rsqrt(const double* in, double* out) {
    // 1. Load 4 doubles
    __m256d v = _mm256_load_pd(in);

    // 2. Vectorized Integer Bit-Hack for initial guess (Quake III style)
    __m256i vi = _mm256_castpd_si256(v);
    __m256i magic = _mm256_set1_epi64x(0x5fe6ec85e7de30daLL);
    __m256i vi_half = _mm256_srli_epi64(vi, 1);
    __m256i yi = _mm256_sub_epi64(magic, vi_half);
    ...
    // 3. Vectorized Newton-Raphson (Iteration 1)
    __m256d half = _mm256_set1_pd(0.5);
    __m256d three_half = _mm256_set1_pd(1.5);
    __m256d x_half = _mm256_mul_pd(v, half);
    ...
    // 4. Vectorized Newton-Raphson (Iteration 2 for 1e-6 precision)
    y2 = _mm256_mul_pd(y, y);
    sub = _mm256_sub_pd(three_half, _mm256_mul_pd(x_half, y2));
    y = _mm256_mul_pd(y, sub);

    _mm256_store_pd(out, y);
}
#endif
